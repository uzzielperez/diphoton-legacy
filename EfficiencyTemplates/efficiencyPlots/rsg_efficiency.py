...
def readCSV_eff(inputfile):
	...
	pattern = "rsg_([^(]*)_"
        width = re.findall(pattern, inputfile)[0]
        print width
        MC_total_dict = MC_totals_dict[width]
	...
	return mass, e_barrel, e_EBorBE, etotal, width
...
widthnum = width.replace("kMpl", "0").replace("00", "0.0")
print "width", widthnum
#leg.SetHeader(r"#frac{#Gamma_{x}}{m_{x}} = 0.01")
#leg.SetHeader(r"#tilde{#kappa} = 0.01")
leg.SetHeader(r"#tilde{#kappa} = %s" %(widthnum))
...
c1.SaveAs("rsg_efficiency%s.pdf" %(width) )
